...
def generate_video(
    ...
    try:
        ...
        try:
            ...
            try:
                image_clip = ImageClip(background_image_path).with_duration(audio_clip.duration)
            except AttributeError:
                try:
                    image_clip = ImageClip(background_image_path).set_duration(audio_clip.duration)
                except AttributeError:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("Cannot set audio on video clip - MoviePy version incompatibility")
        ...
        while True:
            ...
            if status_data["status"] == "done":
                ...
                if video_response.status_code == 200:
                    ...
                    logger.info(f"Successfully generated video with D-ID: {output_path}")
                    return True
                else:
                ...
                logger.error(f"D-ID generation error: {status_data.get('error', 'Unknown error')}")
                return False
