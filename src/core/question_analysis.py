...
def analyze_question_maturity(question: str, groq_client=None) -> Dict[str, Any]:
    ...
    try:
        ...
        response = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are an expert in assessing programming knowledge levels. Always respond with valid JSON."},
        ...
        if json_match:
            ...
            logger.info(f"Question analysis completed: {analysis['knowledge_level']} level")
            return analysis
        else:
...
def classify_question_type(question: str, groq_client=None) -> Dict[str, Any]:
    ...
    try:
        ...
        response = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "Você é um especialista em classificação de perguntas educacionais. Responda sempre com JSON válido."},
        ...
        if json_match:
            ...
            logger.info(f"Question classified as: {classification.get('type')} with {classification.get('verbosity')} verbosity")
        ...
        return {"type": "technical", "verbosity": "moderate", "style": "educational", "reasoning": "Classification failed, using default"}
...
def check_question_scope(question: str, collection, groq_client=None) -> Dict[str, Any]:
    ...
    if not groq_client:
        ...
        return {"in_scope": True, "confidence": 0.5, "reasoning": "No LLM available for scope validation"}
    ...
    for result in search_results:
        ...
        if 'programação' in content_preview.lower() or 'programming' in content_preview.lower():
    ...
    try:
        ...
        response = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "Você é um especialista em validação de escopo. Responda sempre com JSON válido."},
        ...
        return {"in_scope": True, "confidence": 0.3, "reasoning": "Scope validation failed, defaulting to in-scope"}
...
def analyze_indexed_content(collection, groq_client=None) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, (doc, meta) in enumerate(zip(documents[:10], metadatas[:10])):  # Limit for token efficiency
            ...
            content_samples.append(f"Arquivo: {file_info} (Tipo: {content_type})\nConteúdo: {content_preview}...")
        ...
        response = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "Você é um especialista em análise de conteúdo educacional. Responda sempre com JSON válido."},
                {"role": "user", "content": analysis_prompt}
            ],
            max_tokens=600,
            temperature=0.1
        ...
        if json_match:
            ...
            logger.info(f"Content analysis completed: {len(analysis_result.get('technologies', []))} technologies identified")
