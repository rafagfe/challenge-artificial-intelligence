...
def generate_adaptive_response(
    ...
    try:
        ...
        if not scope_validation.get("in_scope", True):
            logger.info(f"Question out of scope: {scope_validation.get('reasoning', 'Unknown reason')}")
        ...
        }.get(question_type.get("verbosity", "moderate"), "Forneça uma explicação equilibrada.")
        ...
        }.get(question_type.get("style", "educational"), "Use abordagem educacional com conceitos e exemplos.")
        ...
        logger.info(f"Generating adaptive response for {analysis['knowledge_level']} level question...")
        response = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "You are an expert programming educator who adapts content to user knowledge levels and preferences."},
...
def generate_out_of_scope_response(question: str, collection, groq_client=None) -> str:
    ...
    # Format sections
    technologies_text = f"- 💻 **Tecnologias:** {', '.join(technologies)}\n" if technologies else ""
    topics_text = f"- 📚 **Tópicos:** {', '.join(topics)}\n" if topics else ""
    content_types_text = f"- 📖 **Formatos:** {', '.join(content_types)}\n" if content_types else ""
    file_info = f"- 📁 **Documentos indexados:** {file_count}\n"
    
    # Format example questions
    examples_text = "\n".join([f"- \"{q}\"" for q in example_questions[:4]]) if example_questions else "- \"Explique os conceitos disponíveis na base de conhecimento\""
