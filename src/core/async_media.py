...
def generate_media_async(text: str, interaction_id: str, message_index: int, 
                        openai_client, audio_path: Path, video_path: Path,
    ...
    try:
        ...
        logger.error(f"Error in async media generation for interaction {interaction_id}: {e}")
...
def start_media_generation_thread(text: str, interaction_id: str, message_index: int,
                                openai_client, audio_path: Path, video_path: Path,
                                states_path: str) -> threading.Thread:
    """
    ...
    thread = threading.Thread(
        ...
        args=(text, interaction_id, message_index, openai_client, audio_path, video_path, states_path),
        daemon=True
    )
    thread.start()
