...
def display_configuration_panel(config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Display configuration panel in sidebar.
    ...
    with st.sidebar.expander("⚙️ Configurações"):
        language = st.selectbox(
            "Idioma",
            ["pt-br", "en", "es"],
            index=0 if not config else ["pt-br", "en", "es"].index(config.get("language", "pt-br"))
        )
        
        temperature = st.slider(
            "Temperatura",
            0.0, 1.0,
            config.get("temperature", 0.7) if config else 0.7
        )
        
        max_tokens = st.number_input(
            "Máximo de Tokens",
            value=config.get("max_tokens", 1000) if config else 1000
        )
