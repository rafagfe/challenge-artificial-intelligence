...
def show_text(message_index: int) -> None:
    ...
    if "messages" in st.session_state and message_index < len(st.session_state.messages):
...
def generate_video_wrapper(text: str = None, audio_path: str = None, interaction_id: str = None) -> str:
    ...
    for i, message in enumerate(st.session_state.messages):
        with st.chat_message(message["role"]):
            ...
            if interaction_id and paths.get("states_path"):
                ...
                media_status = check_media_status(interaction_id, str(paths["states_path"]))
            ...
            if i == len(st.session_state.messages) - 1:
                ...
                if not message.get("show_text"):
                    col1.button("📄 Texto", key=f"text_{i}", on_click=show_text, args=(i,))
                
                # Audio button with real-time status
                if message.get("audio_path") and message.get("audio_ready"):
                    if not message.get("show_audio"):
                        col2.button("🎧 Áudio", key=f"audio_{i}", on_click=show_audio, args=(i,))
                    else:
                        col2.button("🎧 Áudio ✅", key=f"audio_{i}_shown", disabled=True)
                elif message.get("audio_ready"):
                    col2.button("🎧 Áudio ✅", key=f"audio_{i}_ready", on_click=show_audio, args=(i,))
                else:
                ...
                if message.get("video_path") and message.get("video_ready"):
                    if not message.get("show_video"):
                        col3.button("🎬 Vídeo", key=f"video_{i}", on_click=show_video, args=(i,))
                    else:
                        col3.button("🎬 Vídeo ✅", key=f"video_{i}_shown", disabled=True)
                elif message.get("video_ready"):
                    col3.button("🎬 Vídeo ✅", key=f"video_{i}_ready", on_click=show_video, args=(i,))
                else:
                    col3.button("🎬 Vídeo ⏳", key=f"video_{i}_loading", disabled=True)

    # Auto-refresh if there are pending media generations
    pending_media = any(
        msg.get("interaction_id") and 
        (not msg.get("audio_ready", True) or not msg.get("video_ready", True))
        ...
        with st.spinner("Analisando sua pergunta e buscando informações..."):
            ...
            if "🤖" in response_text:  # Handle error from generator
                ...
                if paths.get("states_path"):
                    ...
                    logger.info(f"Started async media generation thread for interaction {interaction_id}")
