    ...
    **Converse com o assistente inteligente para aprofundar seus conhecimentos!**
    ...
    with st.spinner("Inicializando e verificando conteúdo..."):
        if "db_initialized" not in st.session_state:
            ...
            # Check if content needs re-indexing
            from core.indexing import get_resources_state, load_index_state, save_index_state, setup_chromadb, process_all_files, index_documents
            ...
            if current_resources_state != last_indexed_state:
                logger.info("Resource files have changed or state file not found. Re-indexing...")
                ...
                documents = process_all_files(str(paths["resources_path"]), clients.get("groq"))
                if documents:
                    index_documents(collection, documents)
...
def generate_media_content(content, audio_enabled=False, video_enabled=False, api_keys=None):
    """
